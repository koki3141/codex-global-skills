...
- Codexがnative capabilityを提供した後も、同じjobのmanual emulatorをactive global skillとして残さない。user-specific処理だけをnative extension pointへ移す。
...
`RETIRE`は、同じjobをCodex組み込み機能が現在提供し、能力gapなしに移行できる場合に使う。

1. native replacementとuser-specific差分を確認する。
2. 差分があればnative hook、config、plugin、別名profileへ最小限だけ移す。
3. obsolete entrypoint、references、examplesを削除する。
4. generated index、audit、運用ノートを更新する。
5. stale linkと旧triggerが残っていないことを確認する。
